...
describe("Metalsmith plugin", () => {
    ...
    const deleteOneFile = function (done) {
        ...
        Metalsmith(__dirname)
            ...
            .use(blc())
            .build((err) => {
                assert.ok(actualNumFiles === numTestFiles);
                assert.ok(err);
                done();
            });
    };
    ...
    for (
        let i = 0, end = numTestFiles, asc = end >= 0;
        asc ? i < end : i > end;
        asc ? i++ : i--
    ) {
        it(
            "should throw an error when there are broken links",
            deleteOneFile.bind(i)
        );
    }
            ...
            .use((files) => {
                files["testfile.html"] = {
                    contents: Buffer.from(
                        '<a href="https://davidxmoody.com/">Link to my blog</a>'
                    )
                };
    ...
    it("should allow links matching options.allowRegex", (done) => {
        Metalsmith(__dirname)
            ...
            .use((files) => {
                files["testfile.html"] = {
                    contents: Buffer.from(
                        '<a href="/nth/specialregex-2/index.html">Regex link</a>'
                    )
                };
            })
    ...
    it("should allow anchors when allowAnchors is set", (done) => {
        Metalsmith(__dirname)
            ...
            .use((files) => {
                files["testfile.html"] = {
                    contents: Buffer.from('<a name="anchorname">anchor</a>')
                };
            })
    ...
    it("should allow anchors using the id tag when allowAnchors is set ", (done) => {
        Metalsmith(__dirname)
            ...
            .use((files) => {
                files["testfile.html"] = {
                    contents: Buffer.from('<a id="anchorname">anchor</a>')
                };
            })
            .use(blc({ allowAnchors: true }))
            .build((err) => {
    ...
    it("should not allow links with a name and href attribute if the href attribute is incorrect", (done) => {
        Metalsmith(__dirname)
            ...
            .use((files) => {
                files["testfile.html"] = {
                    contents: Buffer.from(
                        '<a href="broken" name="anchorname">anchor</a>'
                    )
                };
            })
            .use(blc({ allowAnchors: true }))
    ...
    it("should throw an error for links to dirs when allowRedirects is not set", (done) => {
        Metalsmith(__dirname)
            ...
            .use((files) => {
                files["testfile.html"] = {
                    contents: Buffer.from('<a href="/dir2">would redirect</a>')
                };
            })
    ...
    it("should throw an error for links that would be valid if not for a baseURL", (done) => {
        Metalsmith(__dirname)
            ...
            .use((files) => {
                files["testfile.html"] = {
                    contents: Buffer.from(
                        '<a href="/a.html">would be valid of not for baseURL</a>'
                    )
                };
            })
